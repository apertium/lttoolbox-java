...
public class IOUtils {
    ...
    public static void setBasePathAndClassLoader(String basePath, ClassLoader classLoader) throws Exception {
        ...
        if (!f.isDirectory()) throw new Exception(basePath + " is not a directory");
        parent = f;
        loader = classLoader;
        zip = null;
    }
    ...
    public static String[] listFilesWithExtension(String extension) {
        ...
        if (zip != null) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (entry.getName().endsWith(ext)) list.add(entry.getName());
            }
            ...
            ArrayList<String> list = new ArrayList();
            File dir = parent;
            if (dir.exists() && dir.isDirectory()) for (File f : dir.listFiles(filter)) list.add(f.getPath());
    ...
    public static byte[] loadByteArray(String filename) throws FileNotFoundException, IOException {
        ...
        if (zip != null || (loader != null && parent == null)) {
            ...
            int b;
            while ((b = is.read(buffer, 0, 1024)) != -1) bos.write(buffer, 0, b);
            bos.flush();
            bos.close();
            is.close();
    ...
    */
    public static ByteBuffer inputStreamToByteBuffer(InputStream input) throws IOException {
      ...
      int n;
      while ( (n=input.read(buf))!=-1) bas.write(buf, 0, n);
      input.close();
      buf = bas.toByteArray();
    ...
    public static InputStream openInFileStream(String filename)
        ...
        if (zip != null) {
            try {
                ...
                if (entry == null) throw new FileNotFoundException("File: " + filename);
                bis = new BufferedInputStream(zip.getInputStream(entry));
                if (DEBUG) System.err.println("openInFileStream in ZIP "+filename);
            ...
            if (is == null) throw new FileNotFoundException("File: " + filename);
            bis = new BufferedInputStream(is);
            if (DEBUG) System.err.println("openInFileStream in ClassLoader "+filename);
        }
        else {
            File file = null;
            try {
                ...
                if (DEBUG) System.err.println("openInFileStream File "+filename);
