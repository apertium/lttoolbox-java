...
public class Translator {
    ...
    public static void setJarAsBase() throws Exception {
        ...
        try {
            ...
            if (modeFiles.length != 1) modeId = null;
        } catch (Exception e) {
    ...
    public static void setBase(String filename) throws Exception {
        if (filename.equals(base)) return;
        ...
        else throw new Exception("Invalid base file.");
    }
    ...
    public static String getTitle(String id) {
        ...
        if (id.endsWith(".mode") || id.endsWith(".jnlp")) id = id.substring(0, id.length() - 5);
        else if (id.endsWith(".jar") || id.endsWith(".zip")) id = id.substring(0, id.length() - 4);
        ...
        for (int i = 0; i < pairs.length; i++) {
            ...
            if (pair.length < 2 || pairs.length > 1 && (pair.length > 2 || pair[0].contains("_") || pair[1].contains("_"))) continue;
            for (int j = 0; j < pair.length; j++) pair[j] = pair[j].trim();
            boolean found = false;
            ...
            if (!found) unidirectionalPairs.add(pair);
        }
        if (unidirectionalPairs.isEmpty() && bidirectionalPairs.isEmpty()) return id;
        else {
    ...
    private static String getTitleForCode(String code) {
        ...
        if (code.equals(title)) {
            if (codeToTitle == null) initCodeToTitle();
