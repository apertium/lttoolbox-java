...
public class ApertiumTransferCompile {
  ...
  public static File doMain(File txFile, File targetClass, File javaSource,
    ...
    if (!cp.exists())
      cp = new File("dist/lttoolbox.jar");
    if (!cp.exists())
      cp = new File("/usr/local/share/apertium/lttoolbox.jar");
    if (!cp.exists())
      cp = new File("/usr/share/apertium/lttoolbox.jar");
    if (!cp.exists())
      cp = new File("dist/lttoolbox.jar"); // fall back to this, to give the best suggestion below
