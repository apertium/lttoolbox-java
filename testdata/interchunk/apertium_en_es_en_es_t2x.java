...
public class apertium_en_es_en_es_t2x extends GeneratedTransferBase
{
	...
	/** MG: caldria treure totes les refer?ncies a ifip en les regles  */
	...
	/** 'Per determinar', per a SN UNDET que han de passar a determinats, transformaci? que far? el postchunk detectant els PDET.  */
	...
	private void macro_concord_SNcoo_SV(Writer out, TransferWord word1, String blank1, TransferWord word2, String blank2, TransferWord word3) throws IOException
	{
		...
		/** per a 2 sintagmes nominals, coordinats entre s?. El nombre del SV ser? sempre pl. 
Arreglar si pot ser alguns errors de concordan?a encara:
 ...
 */
	...
	/** REGLES L?XIQUES: definides en primer lloc perqu? si no es posen per altres regles  */
	// REGLA: SN like SN
	public void rule0__SN__vblike__SNnorel(Writer out, TransferWord word1, String blank1, TransferWord word2, String blank2, TransferWord word3) throws IOException
	{
		...
		/**  quan el subjecte ?s un pronom... */
		if ((word1.tl(attr_lem).equalsIgnoreCase("prnsubj")
    || word1.tl(attr_lem).equalsIgnoreCase("prnsubj2")))
		{
			/** ...creo un chunk que tindr? un pronom feble  */
		...
		/** Assigna g?nere i nombre al segon SN (GD i ND)  */
		if (word3.tl(attr_gen).equals("<GD>"))
		{
			word3.tlSet(attr_gen, "<m>");
		}
	...
	// REGLA: SN adv adv like SN (igual que l'anterior per? amb dos adverbis, per a tractar: 'she really does not like potatoes'
	...
	public void rule3__SN__pro_vblike(Writer out, TransferWord word1, String blank1, TransferWord word2) throws IOException
	{
		...
		/**  canvia el nom del chunk per poder-hi treballar m?s facilment al t3x */
	...
	// REGLA: SN adv adv like-prn (igual que regla anterior per? amb dos adverbis)
	...
	/** NOTA MG: la millor manera de fer el canvi de 'ser' per 'haver' hauria estat enlla?ar la categoria del verb (vbser, vblex, vbhaver, vbmod) en les tags del chunk en el t1x, i aqu? canviar l'etiqueta, de manera que el canvia hauria estat autom?tic i no hauria fet falta fer operacions complicades en el t3x per a canviar les etiquetes segons el nom de chunk. Tanmateix, en el t1x hem posat la variable 'tipus_verb' com a etiqueta del chunk que pot contenir 'pron' i 'like' a part dels tipus de verb normals (vbser, vbhaver, vbmod, vblex), la qual cosa impossibilita fer l'enlla? correctament. Amb m?s temps caldria canviar la manera de passar la informaci? de 'pron' i 'like' en el segon transfer de manera que fos possible fer l'enlla? del tipus de verb.  */
	// REGLA: there BE SN
	...
	// REGLA: there BE (sense SN clar a continuaci?)
	...
	// REGLA: there ADV BE (sense SN clar a continuaci?)
	...
	// REGLA: there ADV ADV BE (sense SN clar a continuaci? - There really can not be)
	...
	public void rule15__SN__SV(Writer out, TransferWord word1, String blank1, TransferWord word2) throws IOException
	{
		...
		{
			macro_f_bcond(out, word1);
			/** el pronom no l'envio, nom?s el 2n chunk  */
			{
				String myword = 
	...
	// REGLA: SN and SN ADV ADV SV (com l'anterior per? dos adv: 'You and your father really can not say')
	...
	// REGLA: SN GEN SN ADV ADV SV (com l'anterior per? dos adv)
	...
	// REGLA: SN SV SA (they live/are happy - viuen feli?os (BCN s'ha modificat)
	...
	public void rule26__SN__SV__SA__cnjcoo__SA(Writer out, TransferWord word1, String blank1, TransferWord word2, String blank2, TransferWord word3, String blank3, TransferWord word4, String blank4, TransferWord word5) throws IOException
	{
		...
		/** Assigna g?nere i nombre a adjectiu (GD i ND)  */
		macro_determina_nbr_gen_SN(out, word1);
		if (word3.tl(attr_nbr).equals("<ND>"))
		{
			word3.tlSet(attr_nbr, var_nombre);
	...
	// REGLA: SN adv adv SV SA (com l'anterior per? amb dos adverbis)
	...
	// REGLA: SN and SN ADV ADV SV SA (com l'anterior per? amb dos adv)
	...
	// REGLA: SN Vcop SN (my sister is teacher - la meva germa ?s mestra
    ...
    || word1.tl(attr_gen).equals("<nt>")))
		...
		/** aqu? enviem el primer SN tant si ?s pronom subjecte com si no.  */
		{
			String myword = 
			         word1.tl(attr_whole)
			         ;
    ...
    && word1.tl(attr_nbr).equals(word3.tl(attr_nbr))))
			...
			macro_f_bcond(out, word1);
			/** el pronom no l'envio, nom?s el 2n i 3r chunk  */
			{
				String myword = 
				         TransferWord.copycase(word1.sl(attr_lem), word3.tl(attr_lem))
				         +word3.tl(attr_tags)
	...
	// REGLA: reporting VERBS (BCN: per afegir QUE quan tenim un 'reporting' werb seguit d'una cl?usula: she said they would go home -- va dir que anirien casa. Falta fer el mateix per quan tenim: SN Vcop SA; SN ADV SV; SN SV SA; a la segona part )
	public void rule38__SN__vbreporting__SNnorel__SV(Writer out, TransferWord word1, String blank1, TransferWord word2, String blank2, TransferWord word3, String blank3, TransferWord word4) throws IOException
	{
	  ...
	  <with-param pos="1"/>
	</call-macro> ens quedem amb els valors del SN 3(?) ara la concordan?a entre el SN 3 i el SV 4  */
		macro_concord_SN_SV(out, word3, blank3, word4);
		macro_posadet_SN(out, word3);
		if (word4.tl(attr_temps).equals("<inf>"))
		{
    ...
    || word1.tl(attr_gen).equals("<nt>")))
		...
		/** Assigna g?nere i nombre a l'adjectiu  */
		macro_determina_nbr_gen_SN(out, word1);
		if (word5.tl(attr_nbr).equals("<ND>"))
		{
			word5.tlSet(attr_nbr, var_nombre);
	...
	public void rule41__SN__pr__SNnorel__SV__SA__cnjcoo__SA(Writer out, TransferWord word1, String blank1, TransferWord word2, String blank2, TransferWord word3, String blank3, TransferWord word4, String blank4, TransferWord word5, String blank5, TransferWord word6, String blank6, TransferWord word7) throws IOException
	{
		...
		/**  quan el segon SN ?s GD o ND (p. ex. quan tenim 'of which' en el segon SN ens ve sense genere i nombre)  */
		if (word3.tl(attr_gen).equals("<GD>"))
		{
			if (!(word1.tl(attr_gen).equals("<mf>")
    || word1.tl(attr_gen).equals("<nt>")))
	...
	public void rule43__SN_nopron__SVGER(Writer out, TransferWord word1, String blank1, TransferWord word2) throws IOException
	{
		...
		macro_posadet_SN(out, word1);
		word2.tlSet(attr_temps, "<pri>");
		/** Aqu?  SN no ?s mai pronom subjecte, la categoria no l'inclou  */
		if (word1.tl(attr_gen).equals("<GD>"))
		{
			word1.tlSet(attr_gen, "<m>");
		}
		...
		/** el gerundi sempre ?s PD i ND, ara cal assignar-li persona p3 i nbr del SN  */
		word2.tlSet(attr_pers, "<p3>");
		if (!word1.tl(attr_nbr).equals("<sp>"))
		{
			word2.tlSet(attr_nbr, word1.tl(attr_nbr));
	...
	// REGLA: SA (per assignar g?nere i nombre si hi ha GD i ND que no s'han pogut determinar)
	...
	// REGLA: SN (per assignar g?nere i nombre si hi ha GD i ND que no s'han pogut determinar, com passa en alguns pronoms: both, etc.)
	...
	// REGLA: DET (per assignar g?nere i nombre si hi ha GD i ND que no s'han pogut determinar, com passa en alguns pronoms: both, etc.)
	...
	// REGLA: NUM (per assignar g?nere si hi ha GD)
	...
	// REGLA: RELNN (per assignar g?nere i nombre si hi ha GD i ND que no s'han pogut determinar (whom).)
